...
def afk(update: Update, context: CallbackContext):
    ...
    if len(args) >= 2:
        ...
        if len(reason) > 100:
            reason = reason[:100]
            notice = "\nAlasan AFK kamu dipendekkan jadi 100 karakter."
    else:
        reason = ""
    ...
    try:
        update.effective_message.reply_text(f"{fname} sedang pergi!{notice}")
    except BadRequest:
        pass
...
def check_afk(update, context, user_id, fst_name, userc_id):
    if sql.is_afk(user_id):
        ...
        if int(userc_id) == int(user_id):
            return
        if not user.reason:
            res = f"{fst_name} lagi offline"
            update.effective_message.reply_text(res)
        else:
            res = f"{html.escape(fst_name)} lagi offline.\Alasan: <code>{html.escape(user.reason)}</code>"
